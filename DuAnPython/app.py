...
@app.route("/")
def index():
    # Lấy từ khóa tìm kiếm và lọc trạng thái
    s = request.args.get("search", "")
    status = request.args.get("status", "")

    query = Post.query

    # 1. Tìm kiếm theo tiêu đề (Skill 3 - Mục 2)
    if s:
    ...
    # Nếu không phải Admin, chỉ cho xem các bài đã "approved" (Phân quyền cơ bản)
    if session.get('role') != 'admin' and not status:
        query = query.filter(Post.status == "approved")

    posts = query.all()
    categories = Category.query.all()

    # Thống kê nhanh để hiển thị trên Dashboard (Skill 4)
...
@app.route("/stats")
def stats():
    # 1. Lấy dữ liệu cho biểu đồ tròn (m đã có)
    pending = Post.query.filter_by(status="pending").count()
    approved = Post.query.filter_by(status="approved").count()

    # 2. THÊM ĐOẠN NÀY: Lấy dữ liệu cho biểu đồ cột (Danh mục)
    posts = Post.query.all()
    # Đếm số bài viết theo từng tên danh mục
    from collections import Counter
    category_names = [p.category.name for p in posts if p.category]
    category_count = Counter(category_names)

    # 3. Truyền tất cả biến sang stats.html
    return render_template(
        "stats.html",
        pending=pending,
        approved=approved,
        labels=list(category_count.keys()), # Tên các danh mục
        data=list(category_count.values())  # Số lượng bài tương ứng
    )
